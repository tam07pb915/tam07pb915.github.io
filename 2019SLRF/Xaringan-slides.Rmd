...
* Detection of ungramaticality ≠ use of grammatical information (Trenkic et al., 2014; Vainio, Pajunen, & Hyona, 2016) 
...
library(lme4)
#library(here)
dat.JLE.one<-read.csv("C:/Users/Yu/Dropbox (Kansai University)/R/Dissertation/JLE-one.csv",sep = ",",header = T)
dat.JLE.one<-dat.JLE.one[,-1]
dat.JLE.two <-read.csv("C:/Users/Yu/Dropbox (Kansai University)/R/Dissertation/JLE-two.csv",sep = ",",header = T)
dat.JLE.two<-dat.JLE.two[,-1]
#head(dat.JLE.one)
```

```{r,message=FALSE,eval=F,echo=F}
#Change factor level
dat.JLE.one$condition<-factor(dat.JLE.one$condition, levels = c("common-sg","collective","common-pl","pl-dominant"))
dat.JLE.one$CEFR<-factor(dat.JLE.one$CEFR, levels = c("A2","B1","B2","C1"),ordered = T)
dat.JLE.two$condition<-factor(dat.JLE.two$condition, levels = c("the-pl","a-sg","one-sg","the-sg"))
dat.JLE.two$CEFR<-factor(dat.JLE.two$CEFR, levels = c("A2","B1","B2","C1"),ordered = T)

#Change coding
c4<-contr.treatment(4)
coding4<-matrix(rep(1/4,12),ncol=3)
simple4<-c4-coding4
contrasts(dat.JLE.one$condition)<-simple4
contrasts(dat.JLE.two$condition)<-simple4
```
...
```{r,echo=FALSE, message=FALSE,eval=F}
setwd()
dat.NS.one<-read.csv("C:/Users/Yu/Dropbox (Kansai University)/R/DissertationNS-one.csv",sep = ",",header = T)
dat.NS.one[,-1] ->dat.NS.one
dat.NS.two <-read.csv("C:/Users/Yu/Dropbox (Kansai University)/R/Dissertation/NS-two.csv",sep = ",",header = T)
dat.NS.two[,-1] ->dat.NS.two
```

```{r,echo=FALSE, message=FALSE,eval=F}
dat.NS.one$condition<-factor(dat.NS.one$condition, levels = c("common-sg","collective","common-pl","pl-dominant"))
dat.NS.two$condition<-factor(dat.NS.two$condition, levels = c("common-sg","collective","common-pl","pl-dominant"))
#Change coding
#c4<-contr.treatment(4)
#coding4<-matrix(rep(1/4,12),ncol=3)
#simple4<-c4-coding4
contrasts(dat.NS.one$condition)<-simple4
contrasts(dat.NS.two$condition)<-simple4
```
...
# Summary of the fiexed effects (L2,one-word)
library(sjPlot)
tab_model(JLE[[1]],show.r2 = F,show.icc = F, show.re.var=F, pred.labels = "")
```
...
## Two-word condition
```{r, message=FALSE,eval=F,echo=F}
JLE[[2]]<-glmer(RT ~ s.pres.order+condition+(1+condition|subject)+(1+condition|itemID),data=slj0926_3_two_2,family = inverse.gaussian(link = "identity"),glmerControl(optimizer=c("bobyqa","Nelder_Mead"),optCtrl = list(maxfun=2e5)))

JLE[[3]]<-update(JLE[[2]],.~.-(1+condition|subject)-(1+condition|itemID)+(0+condition|subject)+(1|subject)+(0+condition|itemID)+(1|itemID))#(0+condition|subject)+(1|subject)+(0+condition|itemID)+(1|itemID)
```
...
### <u>Responses to the plural-dominant plurals</u>
  * Maybe because of NS processed highly frequent plural forms as whole words .small[(e.g., Baayen, Dijkstra, & Schreuder, 1997; Beyersmann, Dutton, Amer, Schiller, & Biedermann, 2015; Biedermann, Beyersmann, Mason, & Nickels, 2013; New, Brysbaert, Segui, Ferrand, & Rastle, 2004 )]
  * Japanese L2 learners' lexical processing of the plural dominant plurals were not accelerated .small[(Tamura, Morita, & Nishimura, 2016)]

---
# Comparison between NS and L2 (contd)
* Both groups did not show interference in the case of _the_ + pl
<br/>-> .red[unmarkedness] of grammatical singularity? .small[(e.g., Berent et al., 2005; Bock & Cutting, 1993; Eberhard, 1997)]
<br/>
...
* L2: _one_ had an impact -> partly resulting from processing tendency driven by lexical meaning? .small[(cf. Shallow Structure Hypothesis: Clahsen & Felser, 2017)]
...
* Identifying singularity of nouns is difficult -> this could be the reason why third-person singular _-s_ is difficult? .small[(e.g., Bannai, 2011; Ladiere,1998; Shibuya & Wakabayashi, 2008)]
...
<br/>->Need to investigate nouns that can either be countable or uncountable (e.g., _ex)
...
* Identifying the singularity may be difficult if it is without any lexical marker (e.g., _one_)
